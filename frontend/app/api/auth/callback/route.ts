...
export async function GET(request: NextRequest) {
  ...
  if (!tx || request.nextUrl.searchParams.get("state") !== tx.state || !request.nextUrl.searchParams.get("code")) return NextResponse.redirect(new URL("/unauthorized?reason=login", request.url));
  ...
  if (!token.ok) return NextResponse.redirect(new URL("/unauthorized?reason=login", request.url));
  ...
  jar.set("rag_csrf", crypto.randomUUID(), { httpOnly: false, secure: process.env.NODE_ENV === "production", sameSite: "strict", path: "/", maxAge: 28800 });
