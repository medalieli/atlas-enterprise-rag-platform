...
export async function GET(request: NextRequest) {
  ...
  if (!tx || request.nextUrl.searchParams.get("state") !== tx.state || !request.nextUrl.searchParams.get("code")) return NextResponse.redirect(new URL("/unauthorized?reason=login", origin));
  ...
  if (!token.ok) return NextResponse.redirect(new URL("/unauthorized?reason=login", origin));
